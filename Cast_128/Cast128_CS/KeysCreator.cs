...
namespace Cast128_CS
{
    class KeysCreator
    {
        ...
        public KeysCreator(String mainKey)
        {
            ...
            if (!val)
            {
                throw new System.ArgumentException("Too small or too big argumet", "mainKey");
            }
            toBinArray();

            x0x1x2x3 = Convert.ToUInt32(binKey.Substring(0, 32), 2);
            x4x5x6x7 = Convert.ToUInt32(binKey.Substring(32, 32), 2);
            x8x9xAxB = Convert.ToUInt32(binKey.Substring(64, 32), 2);
            xCxDxExF = Convert.ToUInt32(binKey.Substring(96, 32), 2);

            SeparateByte(ref x0, ref x1, ref x2, ref x3, x0x1x2x3);
            SeparateByte(ref x4, ref x5, ref x6, ref x7, x4x5x6x7);
            SeparateByte(ref x8, ref x9, ref xA, ref xB, x8x9xAxB);
            SeparateByte(ref xC, ref xD, ref xE, ref xF, xCxDxExF);

            for (int j = 0; j < 2; j++)
                for (int i = 0; i < 9; i = i + 8)
                {
                    z0z1z2z3 = x0x1x2x3 ^ S5[xD] ^ S6[xF] ^ S7[xC] ^ S8[xE] ^ S7[x8];
                    SeparateByte(ref z0, ref z1, ref z2, ref z3, z0z1z2z3);

                    z4z5z6z7 = x8x9xAxB ^ S5[z0] ^ S6[z2] ^ S7[z1] ^ S8[z3] ^ S8[xA];
                    SeparateByte(ref z4, ref z5, ref z6, ref z7, z4z5z6z7);

                    z8z9zAzB = xCxDxExF ^ S5[z7] ^ S6[z6] ^ S7[z5] ^ S8[z4] ^ S5[x9];
                    SeparateByte(ref z8, ref z9, ref zA, ref zB, z8z9zAzB);

                    zCzDzEzF = x4x5x6x7 ^ S5[zA] ^ S6[z9] ^ S7[zB] ^ S8[z8] ^ S6[xB];
                    SeparateByte(ref zC, ref zD, ref zE, ref zF, zCzDzEzF);

                    K[j, i] = S5[z8] ^ S6[z9] ^ S7[z7] ^ S8[z6] ^ S5[z2];
                    K[j, i + 1] = S5[zA] ^ S6[zB] ^ S7[z5] ^ S8[z4] ^ S6[z6];
                    K[j, i + 2] = S5[zC] ^ S6[zD] ^ S7[z3] ^ S8[z2] ^ S7[z9];
                    K[j, i + 3] = S5[zE] ^ S6[zF] ^ S7[z1] ^ S8[z0] ^ S8[zC];
                    ...
                    K[j, i + 4] = S5[x3] ^ S6[x2] ^ S7[xC] ^ S8[xD] ^ S5[x8];
                    K[j, i + 5] = S5[x1] ^ S6[x0] ^ S7[xE] ^ S8[xF] ^ S6[xD];
                    K[j, i + 6] = S5[x7] ^ S6[x6] ^ S7[x8] ^ S8[x9] ^ S7[x3];
                    K[j, i + 7] = S5[x5] ^ S6[x4] ^ S7[xA] ^ S8[xB] ^ S8[x7];
                }
        ...
        void SeparateByte(ref uint b0, ref uint b1, ref uint b2, ref uint b3, uint b0b1b2b3)
        {
            ...
            b3 = b0b1b2b3 & 255;
            b2 = (b0b1b2b3 & 65280) >> 8;
            b1 = (b0b1b2b3 & 16711680) >> 16;
            b0 = (b0b1b2b3 & 4278190080) >> 24;
        }
        ...
        String finalGetBinaryValue(String num)
        {
            finArr = "";
        ...
        String getBin(String num)
        {
            ...
            {
                //numArr[i] = Character.getNumericValue(num[i]);
                numArr[i] = Convert.ToInt32(new string(num[i], 1));
            }
            int p = 0;
            String quo = "";
            int rem = numArr[0];
            ...
            finArr = rem.ToString() + finArr;
            //finArr = Int32.toString(rem) + finArr;
            return quo;
        }

        public static uint[] S5 = {
